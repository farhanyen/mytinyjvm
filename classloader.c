...
void parse_constant(ByteBuf *buf, cp_info *con) {
    ...
    switch(tag) {
        case CONSTANT_Class:
            con->class_info.name_index = read_u2(buf);
            break;
        case CONSTANT_Fieldref:
            con->fieldRef_info.class_index = read_u2(buf);
            con->fieldRef_info.name_and_type_index = read_u2(buf);
            break;
        case CONSTANT_Methodref :
            con->methodRef_info.class_index = read_u2(buf);
            con->methodRef_info.name_and_type_index = read_u2(buf);
            break;
        case CONSTANT_InterfaceMethodref:
            con->interfaceMethodRef_info.class_index = read_u2(buf);
            con->interfaceMethodRef_info.name_and_type_index = read_u2(buf);
            break;
        case CONSTANT_String:
            con->string_info.string_index = read_u2(buf);
            break;
        case CONSTANT_Integer:
            con->integer_info.bytes = read_u4(buf);
            break;
        case CONSTANT_Float:
            con->float_info.bytes = read_u4(buf);
            break;
        case CONSTANT_Long:
            con->longHigh_info.bytes = read_u4(buf);
            break;
        case CONSTANT_Double:
            con->doubleHigh_info.bytes = read_u4(buf);
            break;
        case CONSTANT_NameAndType:
            con->nameAndType_info.name_index = read_u2(buf);
            con->nameAndType_info.descriptor_index = read_u2(buf);
            break;
        case CONSTANT_Utf8:
            con->utf8_info.length = read_u2(buf);
            con->utf8_info.bytes = malloc(con->utf8_info.length);
            for (int i = 0; i < con->utf8_info.length; i++) {
                con->utf8_info.bytes[i] = read_u1(buf);
            }
            break;
        case CONSTANT_MethodHandle:
            con->methodHandle_info.reference_kind = read_u1(buf);
            con->methodHandle_info.reference_index = read_u2(buf);
            break;
        case CONSTANT_MethodType:
            con->methodType_info.descriptor_index = read_u2(buf);
            break;
        case CONSTANT_InvokeDynamic:
            con->invokeDynamic_info.bootstrap_method_attr_index = read_u2(buf);
            con->invokeDynamic_info.name_and_type_index = read_u2(buf);
            break;
    }
}

void parse_attribute_info(ByteBuf *buf, attribute_info *attribute) {
    attribute->attribute_name_index = read_u2(buf);
    attribute->attribute_length = read_u4(buf);
    attribute->info = malloc(attribute->attribute_length);
    for (int i = 0; i < attribute->attribute_length; i++) {
        attribute->info[i] = read_u1(buf);
    }
}

void parse_field_info(ByteBuf *buf, field_info *field) {
    field->access_flags = read_u2(buf);
    field->name_index = read_u2(buf);
    field->descriptor_index = read_u2(buf);
    field->attributes_count = read_u2(buf);
...
void parse_exception_table(ByteBuf *buf, exception_table *table) {
    table->start_pc = read_u2(buf);
    table->end_pc = read_u2(buf);
    table->handler_pc = read_u2(buf);
    table->catch_type = read_u2(buf);
}

void parse_method_bytecode(ByteBuf *buf, ByteCode *code) {
    code->max_stack = read_u2(buf);
    code->max_locals = read_u2(buf);
    code->code_length = read_u4(buf);
    code->code = malloc(code->code_length);
    for (int i = 0; i < code->code_length; i++) {
        code->code[i] = read_u1(buf);
    }
    code->exception_table_length = read_u2(buf);
    ...
    code->attributes_count = read_u2(buf);
...
void parse_method_info(ByteBuf *buf, method_info *method, ClassFile *cf) {
    method->access_flags = read_u2(buf);
    method->name_index = read_u2(buf);
    method->descriptor_index = read_u2(buf);
    method->attributes_count = read_u2(buf);
...
int read_class (ByteBuf *buf, ClassFile *cf) {
    u4 magic = read_u4(buf);
    if (magic != 0xcafebabe) {
        printf("magic wrong %x", magic);
        return -1;
    }
    cf->minor_version = read_u2(buf);
    cf->major_version = read_u2(buf);
    cf->constant_pool_count = read_u2(buf);
